...
def main():
    ...
    # Initialize total weights per material
    total_weights = {
        '9k Gold': 0.0,
        '14k Gold': 0.0,
        '18k Gold': 0.0,
    ...
    # Sum up total volume per material
    for component in components:
        volume = component.volume
        for material, density in density_values.items():
            total_weights[material] += (volume * density) / 1000000
    ...
    # Define fonts
    try:
        header_font = ImageFont.truetype("arialbd.ttf", 18)  # Bold for headers
        regular_font = ImageFont.truetype("arial.ttf", 15)   # Regular for values
    except IOError:
        header_font = ImageFont.load_default()
        regular_font = ImageFont.load_default()

    # Define table layout
    start_x, start_y, column_width = 10, 10, 150
    ...
    # Draw rows
    start_y += 30  # Adjust for spacing after header
